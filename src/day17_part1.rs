        ...
        for x in x_min..=x_max {
            for y in y_min..=y_max {
                for z in z_min..=z_max {
                    let an = count_active_neigbours(&grid, x, y, z);
                    // println!(
                    //     "at {x},{y},{z} ({}): {an} active neigbours",
                    //     is_inactive(&grid, x, y, z)
                    // );
                    if is_inactive(&grid, x, y, z) && an == 3
                        || !is_inactive(&grid, x, y, z) && [2, 3].contains(&an)
                    {
                        // println!("add #");
                        new_grid.insert(Point { x, y, z }, '#');
                    }
                }
            }
        }
...
fn count_active_neigbours(grid: &HashMap<Point, char>, x: isize, y: isize, z: isize) -> usize {
    let mut result = 0;
    for xi in x - 1..=x + 1 {
        for yi in y - 1..=y + 1 {
            for zi in z - 1..=z + 1 {
                if !(is_inactive(grid, xi, yi, zi) || xi == x && yi == y && zi == z) {
                    result += 1;
                }
            }
        }
...
fn is_inactive(grid: &HashMap<Point, char>, x: isize, y: isize, z: isize) -> bool {
    if let Some(pt) = grid.get(&Point { x, y, z }) {
        if *pt == '#' {
            return false;
        }
    }
...
fn _print(grid: &HashMap<Point, char>) {
    ...
    for z in z_min..=z_max {
        ...
        for y in y_min..=y_max {
            for x in x_min..=x_max {
                if grid.get(&Point { x, y, z }).is_some() {
                    print!("#");
                } else {
                    print!(".");
                }
